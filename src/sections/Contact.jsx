...
const Contact = () => {
  ...
  return (
    <section 
      id="contact" 
      ref={sectionRef} 
      className="w-full bg-[#050505] py-24 font-sans relative overflow-hidden border-t border-white/5 no-invert"
    >
      {/* 1. GIANT EDITORIAL WATERMARK */}
      <div className="absolute top-10 left-10 text-[15vw] font-black text-white/[0.02] leading-none pointer-events-none uppercase tracking-tighter z-0">
        Connect
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-20 items-start">
          ...
          <div className="contact-fade lg:col-span-5 w-full flex flex-col gap-10">
            ...
            <div className="w-full max-w-sm flex items-center gap-6 p-5 bg-[#0c0c0c] border border-white/10 rounded-3xl group hover:border-[#a8ff35]/30 transition-all duration-500 shadow-2xl relative">
            ...
            <p className="text-zinc-400 font-light text-base md:text-lg leading-relaxed max-w-md border-l-2 border-[#a8ff35]/50 pl-6">
              Ngobrol santai aja, nggak usah kaku. Ceritain idemu, visimu, atau tantangan desain yang lagi kamu hadapin.
            </p>
          </div>
          ...
          <div className="contact-fade lg:col-span-7 w-full">
            <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-2">
              {formFields.map((field) => (
                <div key={field.id} className="group flex flex-col border-b border-white/10 hover:border-[#a8ff35]/30 transition-colors duration-500">
                  ...
                  {field.type === "textarea" ? (
                    <textarea
                      ...
                      rows="3"
                      className="w-full bg-transparent border-none text-white text-base md:text-lg p-1.5 focus:outline-none placeholder:text-zinc-800 transition-colors resize-none font-medium"
                      required
                    />
                  ) : (
                    <input
                      ...
                      onChange={handleChange}
                      placeholder={field.placeholder}
                      className="w-full bg-transparent border-none text-white text-base md:text-lg p-1.5 focus:outline-none placeholder:text-zinc-800 transition-colors font-medium"
                      required
                    />
                  )}
        ...
        {/* 3. FINAL DIRECTORY FOOTER */}
        <div className="contact-fade mt-32 border-t border-white/5 pt-12 flex flex-col md:flex-row md:items-center justify-between gap-6 opacity-40 hover:opacity-100 transition-opacity duration-700">
          <p className="font-mono text-[10px] text-zinc-500 tracking-[0.2em] uppercase">
            © {new Date().getFullYear()} REKY. // GPS: 3.2553° N, 97.1764° E
          </p>
          <div className="flex items-center gap-4">
            {['In ↗', 'Gh ↗', 'Ig ↗'].map((link, idx) => (
